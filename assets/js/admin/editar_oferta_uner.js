...
function registrar() {
	...
	if ($mencion != "" && $cupos_disponibles != "" && $id_area_formacion != "" && $id_usuario_registro != "" && $duracion != "" && $descripcion != "" && $sexo != "" && $cantidad.match(/^[0-9]+$/) != "" && $titularidad != "" && $edad != "") {
        $.ajax({
            ...
            url: base_url + "CofertaUniversidades/update_oferta",
            type: "post",
            beforeSend: function () {
                ...
                if (respuesta.resultado == true) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            // $(location).attr("href", base_url + "admin/ofertasUniversidad");
							if($id_rol == 2){
								$(location).attr("href", base_url + "admin/ofertasUniversidad");
							}else if($id_rol == 3){
...
$("#titularidad").on("change", function () {
	...
	if (expresion.test(titularidad)) {
		...
		$(this).removeClass("is-invalid error-input").addClass("is-valid");
		$("#titularidad").removeClass("is-valid").addClass("is-invalid");
	}
});
...
$("#mencion").on("keyup", function () {
	...
	if (expresion.test(mencion)) {
		...
		$(this).removeClass("is-invalid error-input").addClass("is-valid");
		$("#mencion").removeClass("is-valid").addClass("is-invalid");
	}
});
$("#duracion").on("keyup", function () {
	...
	if (expresion.test(duracion)) {
		...
		$(this).removeClass("is-invalid error-input").addClass("is-valid");
		$("#duracion").removeClass("is-valid").addClass("is-invalid");
	}
});
...
$("#descripcion_oferta").on("keyup", function () {
	...
	if (descripcion_oferta) {
		...
		$(this).removeClass("is-invalid error-input").addClass("is-valid");
		$("#descripcion_oferta").removeClass("is-valid").addClass("is-invalid");
	}
});

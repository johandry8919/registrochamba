(function ($) {
	...
	$("#wizard2").steps({
		...
		onStepChanging: function (event, currentIndex, newIndex) {
			if (currentIndex < newIndex) {
				...
				if (currentIndex === 0) {
					...
					if (razon_social.isValid() && rif.isValid() 
						...
						if(!sector_economico.isValid())
							$('#sector_economico').removeClass('is-valid').addClass('is-invalid  error-input');
			...
			if(!direccion_empresa.isValid())
			$('#direccion_empresa').removeClass('is-valid').addClass('is-invalid  error-input');
		...
		$.ajax({
				...
				url: base_url + "Estructuras/crearEmpresas",
				type: "post",
				beforeSend: function () {
